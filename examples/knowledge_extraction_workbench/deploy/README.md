...
- `knowledge-workbench-offline-0.1.2-linux-amd64.tar`：用于 `uname -m` 显示 `x86_64` 的 Intel/AMD 服务器。
- `knowledge-workbench-offline-0.1.2-linux-arm64.tar`：用于 `uname -m` 显示 `aarch64` 或 `arm64` 的 ARM 服务器。
