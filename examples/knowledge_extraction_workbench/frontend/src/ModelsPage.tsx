...
export function ModelsPage() {
  ...
  return (
    <div className="page models-page">
      ...
      <Notice tone="info">当前完整流程默认使用内置 Fake Model。录入真实连接后，可先执行安全的最小调用测试，再手工挂载到能力卡。</Notice>
...
function ModelModal({ model, onClose, onSaved }: { model: ModelConnection | "new" | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setProvider(editing?.provider || "DeepSeek");
  ...
  return (
    <Modal open={Boolean(model)} title={editing ? "编辑模型连接" : "新增模型连接"} subtitle="密钥仅在本次保存时发送到本机后端；查询接口只返回 has_api_key。" onClose={onClose} footer={<><Button onClick={onClose}>取消</Button><Button kind="primary" onClick={() => void save()} disabled={saving || !name.trim() || !modelName.trim()}>{saving ? "保存中…" : "保存连接"}</Button></>}>
      <div className="form-stack">
        {error && <Notice tone="danger">{error}</Notice>}
        <label>连接名称<span>*</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：DeepSeek 生产连接" /></label>
        <div className="two-fields"><label>Provider<span>*</span><select value={provider} onChange={(event) => setProvider(event.target.value)}><option>DeepSeek</option><option>OpenAI</option><option>OpenRouter</option><option>Anthropic</option><option>SiliconFlow</option><option>DashScope</option><option>InferenceAffinity</option></select></label><label>模型名称<span>*</span><input value={modelName} onChange={(event) => setModelName(event.target.value)} placeholder="填写服务端实际 Model ID" /></label></div>
        <label>API 地址<span>*</span><input value={apiBase} onChange={(event) => setApiBase(event.target.value)} placeholder="https://…" /><small>外部服务必须使用 HTTPS；HTTP 仅允许 localhost / loopback。</small></label>
        ...
        <label className="toggle-field"><span><b>启用连接</b><small>停用后不出现在可挂载模型中</small></span><button className={`switch ${enabled ? "on" : ""}`} onClick={() => setEnabled(!enabled)}><i /></button></label>
      </div>
    </Modal>
  );
}
