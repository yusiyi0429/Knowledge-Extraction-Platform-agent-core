...
当前版本默认使用确定性的 `FakeKnowledgeModel`，无需 API Key 即可完整体验场景探索、知识萃取、对齐修订和资产发布流程。

## 核心能力

- **场景探索**：上传多份素材，按素材轮询和全文分桶公平取样，生成带来源引用的候选场景。
- **场景与素材**：维护目标、子场景和素材角色，支持素材停用、场景归档及新萃取轮次。
- **知识萃取与对齐**：受控 Map/Reduce 萃取、SSE 进度、Markdown 研判文档、人工修订和 AI 差异建议。
- **资产生成与发布**：生成规则 Excel、决策研判链 Markdown、openJiuwen Skill ZIP、QA JSONL 和合成评测 JSONL。
- **能力配置**：7 张按需执行的能力卡，可配置模型、Skill 和少量必要参数，不启动常驻多智能体系统。
...
aiohttp + SQLModel + SQLite
          ...
          ├─ FakeKnowledgeModel（默认）
          └─ openJiuwen Model（可选真实模型）
```
...
首次进入只需要填写本机显示名称。默认 7 项能力均挂载 Fake Model，不需要提供任何真实密钥。
...
Fake Model 用于本地体验和自动化测试。需要接入 DeepSeek 等兼容模型时：

1. 在“模型接入”页面新增 Provider、HTTPS API 地址、模型名称和 API Key。
2. 使用“测试连接”执行不包含素材正文的最小请求。
3. 在“智能体与 Skill”中把所需能力卡切换到该模型。
4. 后续新任务会冻结此次模型和 Skill 配置。

密钥只加密保存在本机数据目录，模型查询接口仅返回 `has_api_key`。请勿把数据目录、密钥文件或 `.env` 提交到版本库。
...
浏览器验收覆盖 `1440×1000` 和 `1280×800`，测试始终使用 Fake Model，不读取真实 API Key。
